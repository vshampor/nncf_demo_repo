...
def train(model, train_loader, val_loader, criterion, class_encoding, epochs, checkpoint_save_dir, args):
    logger.info("\nTraining...\n")
    tb = SummaryWriter(RESULT_DIR / 'tb_logs')
    ...
    for epoch in range(0, epochs):
        ...
        tb.add_scalar("train/mIoU", miou, epoch)
        tb.add_scalar("train/learning_rate", optimizer.param_groups[0]['lr'], epoch)
        ...
        if (epoch + 1) % save_freq == 0 or epoch + 1 == epochs:
            ...
            tb.add_scalar("val/loss", loss, epoch)
